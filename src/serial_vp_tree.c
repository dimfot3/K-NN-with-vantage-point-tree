...
struct vp_point* serial_vp_create(struct points_struct* points, int* idxs,  int n)
{
    if(n == 0)
        return NULL;
    int d = points->dim;
    ...
    node->left = serial_vp_create(points, left_idxs, n_l);
    if(node->left != NULL)
        node->left->parent = node;
    node->right = serial_vp_create(points, right_idxs,  n_r);
    if(node->right != NULL)
