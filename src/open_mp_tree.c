 ...
 * @brief Here is the implementation of Vantage point creation in OpenMP
 **/
...
struct vp_point* openmp_vp_create(struct points_struct* points, int* idxs,  int n, struct vp_point* parrent)
{
    ...
    #pragma omp parallel
    {
      #pragma omp sections nowait
        {
            #pragma omp section
            node->left = openmp_vp_create(points, left_idxs, n_l, node);
            #pragma omp section
            node->right = openmp_vp_create(points, right_idxs,  n_r, node);
        }
    }
